...
({if $c_member.countdown})
<div align="center"><font color="red" size="2">({$c_member.countdown})歳まであと({$countdown_days})日</font></div>
({/if})
...
<a href="({t_url m=ktai a=page_h_confirm_list})&amp;({$tail})"><font color="red">★部活動参加承認待ち({$num_h_confirm_list})名</font></a><br>
({/if})

({if $anatani_c_commu_admin_confirm_list})
<a href="({t_url m=ktai a=page_h_confirm_list})&amp;({$tail})"><font color="red">★部活動管理人交代依頼({$num_anatani_c_commu_admin_confirm_list})件</font></a><br>
({/if})
...
<div>&em_3square;({if $c_commu_topic_list})<a href="#rcommu" accesskey="3"><font color="blue">部活動最新書き込み</font></a><blink>&em_new;</blink>({else})ﾄﾋﾟｯｸ更新({/if})</div>
<div>&em_4square;({if $c_commu_list})<a href="#mcommu" accesskey="4"><font color="blue">My部活動ﾘｽﾄ</font></a>({else})My部活動ﾘｽﾄ({/if})</div>
...
({/if})
<div>&em_6square;<a href="({t_url m=ktai a=page_fh_diary_list})&amp;({$tail})" accesskey="6"><font color="blue">My日記</font></a></div>
...
<div><a href="({t_url m=ktai a=page_h_com_find_all})&amp;({$tail})">部活動検索</a>|<a href="({t_url m=ktai a=page_h_friend_find_all})&amp;({$tail})">ﾒﾝﾊﾞｰ検索</a></div>
<div align="right">
...
<hr>
<a name="rcommu"></a><font color="green">&em_3square;[部活動最新書き込み]</font><br>
...
({/if})
<hr>
<a name="mcommu"></a><font color="green">&em_4square;[参加部活動({if $count_commu})(({$count_commu}))({/if})]</font><br>
({if $c_commu_list})
...
参加していません<br>
({/if})
<div align="right"><a href="({t_url m=ktai a=page_h_com_find_all})&amp;({$tail})">→部活動検索</a><br>
<a href="#top" accesskey="*">▲</a></div>
<hr>
({if $c_friend_list})
