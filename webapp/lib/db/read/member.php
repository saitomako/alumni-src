...
if (! function_exists('days_to_100old')) {
    function days_to_100old($c_member_id) {
        $c_member = db_common_c_member4c_member_id($c_member_id);
        $birth_year = $c_member['birth_year'];
        $birth_month = $c_member['birth_month'];
        $birth_day = $c_member['birth_day'];
        $today = strptime(date("Y-m-d"), "%Y-%m-%d");
        $today_year = $today['tm_year'] + 1900 - 100;
        $today_month = $today['tm_mon'] + 1;
        $today_day = $today['tm_mday'];
        $dt1 = "$birth_year/$birth_month/$birth_day";
        $dt2 = "$today_year/$today_month/$today_day";
        //$dt1 = mktime(0,0,0, $birth_month, $birth_day, $birth_year+100);
        //$dt2 = mktime(0,0,0, $today_month, $today_day, $today_year);
...
if (! function_exists('days_for_countdown')) {
	function days_for_countdown($c_member_id) {
		$ret = 0;
		$c_member = db_common_c_member4c_member_id($c_member_id);
		$countdown = $c_member['countdown'];
		if ($countdown > 0) {
			$birth_year = $c_member['birth_year'] - 100 + $countdown;
			$birth_month = $c_member['birth_month'];
			$birth_day = $c_member['birth_day'];
			$today = strptime(date("Y-m-d"), "%Y-%m-%d");
			$today_year = $today['tm_year'] + 1900 - 100;
