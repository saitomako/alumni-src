...
function _smarty_modifier_delete_link4topic($topic_id,$comment_id) {
    ...
    if(function_exists("smarty_modifier_t_url2pne")){
        ...
        if (empty($db_msg['name']) || $db_msg['event_flag'] == 1) {
            ...
            $link_str = "【" . htmlspecialchars($db_msg['name'], ENT_QUOTES) . "】トピック（部活動：" . htmlspecialchars($db_cmmu['name'], ENT_QUOTES) ."）<br>" . $link_url ."<br>";
        }
    }
    return $link_str;
}
...
function _smarty_modifier_delete_link4event($topic_id,$comment_id) {
    ...
    if(function_exists("smarty_modifier_t_url2pne")){
        ...
        if (empty($db_msg['name']) || $db_msg['event_flag'] != 1) {
            ...
            $db_cmmu = _db_c_commu4c_commu_id($db_msg['c_commu_id']);
            $link_str = "【" . htmlspecialchars($db_msg['name'], ENT_QUOTES) . "】イベント（部活動：" . htmlspecialchars($db_cmmu['name'], ENT_QUOTES) ."）<br>" . $link_url ."<br>";
        }
    }
    return $link_str;
}
...
function _smarty_modifier_delete_link4commu($commu_id) {
    ...
    if(function_exists("smarty_modifier_t_url2pne")){
        ...
        if (empty($db_msg['name'])) {
            if(isKtaiUserAgent()){
                $link_str = '【該当する部活動はありません】';
            }else{
                $link_str = '<span title="commu=' . $commu_id . '" class="bb-red">【該当する部活動はありません】</span>';
            }
        } else {
            $link_str = "【" . htmlspecialchars($db_msg['name'], ENT_QUOTES) . "】部活動<br>" . $link_url ."<br>";
        }
    }
    return $link_str;
}
