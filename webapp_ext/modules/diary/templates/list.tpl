...
<h2>
({if $target_member.c_member_id})
<a href="({t_url m=diary a=page_list})&amp;target_c_member_id=({$target_member.c_member_id})">({$target_member.nickname})</a>さんの({/if})公開日記
({if !$all})
...
<h3 class="green"><a href="({t_url m=diary a=page_detail})&amp;target_c_diary_id=({$item.c_diary_id})">({$item.subject})</a></h3>
